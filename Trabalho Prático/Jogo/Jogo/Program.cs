            ...
            if (File.Exists(ficheiroPontuacao) == true)
            {
                if (existeJogador(nome, ficheiroPontuacao) == true)
                {
                    ...
                    while ((linha = sr.ReadLine()) != null)
                    {
                        ...
                        {
                            // AJUSTE DA PONTUAÇÃO
                            if ((Convert.ToInt16(partes[3]) + pontosJogada) - pontosJogada < 0) // se pontos - pontosJogada < 0
                                pontosJogada = 0;
                            if (pontosJogada == 1)
                            ...
                            Convert.ToString(Math.Round((Convert.ToDouble(partes[3]) + pontosJogada) / (Convert.ToDouble(Convert.ToInt16(partes[2]) + 1)), 2) * 100);    // score final (pontos / nº jogadas)
